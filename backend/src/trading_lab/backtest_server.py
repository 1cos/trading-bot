...
def _available_symbols() -> list[dict]:
    ...
    for f in sorted(DATI_DIR.iterdir()):
        if f.suffix == ".csv" and "_5m" in f.name:
            sym = f.name.replace("_5m.csv", "")
...
def _build_trade_row(r: dict, idx: int, sessions_data: dict) -> dict:
    ...
    return {
        ...
        "direction": str(_g(dr, "direction", "?")),
        "sequence_id": f"SEQ-{idx + 1:03d}",
...
def api_run():
    ...
    try:
        ...
        if timeframe != "5m":
            return jsonify({"error": f"Timeframe {timeframe} not available. Only 5m data exists."}), 400

        # Build sessions from real data
        all_sessions = []
        sessions_data = {}
        for sym in symbols:
            data = _load_symbol_data(sym, timeframe)
            if data.get("error"):
                return jsonify({"error": data["error"]}), 400
            sessions_data[sym] = data
            for date in data["dates"]:
                if start_date and date < start_date:
                    continue
                if end_date and date > end_date:
                    continue
                candles = data["sessions"][date]
                all_sessions.append({
                    "symbol": sym,
                    "date": date,
        ...
        # Build trade rows
        valid_results = [r for r in results if r["detection_status"] == "VALID"]
        ...
        for i, r in enumerate(valid_results):
            ...
            event["sequence_id"] = f"SEQ-{i + 1:03d}"

            # Add sequence validation data
            candles = session["candles"]
            ec = {
                "timeframe_minutes": preset["timeframe_minutes"],
                "timezone": preset["timezone"],
                "session_open": preset["session_open"],
                "orb_start": preset["orb_start"],
                "orb_duration_minutes": preset["orb_duration_minutes"],
                "level_source": preset["level_source"],
                "direction": preset["direction"],
                "tick_size": config["tick_size"],
                "min_displacement_ticks": preset["min_displacement_ticks"],
                "min_penetration_ticks": preset["min_penetration_ticks"],
                "min_close_beyond_level_ticks": preset["min_close_beyond_level_ticks"],
                "consecutive_orb_closes": preset["consecutive_orb_closes"],
            }
            try:
            ...
            if d_idx > 0:
                prev_c = sym_data["sessions"][sym_dates[d_idx - 1]]
